...
async function runDailyAnalysis({ skipTelegram = false }: { skipTelegram?: boolean } = {}) {
  ...
  try {
    ...
    const tasks: Promise<unknown>[] = [runPortfolioAdvice(), runAccountAdvice()]
    if (!skipTelegram) {
    ...
    const accountResult = settled[skipTelegram ? 1 : 2]
    if (accountResult.status === 'rejected') console.error('[계좌조언] 생성 실패:', (accountResult as PromiseRejectedResult).reason)
