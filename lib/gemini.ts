...
2. MACD↓(데드크로스) 종목은 추천 금지 (거래유형 무관)
3. 추세↓(하락추세) 종목은 단타·스윙·중기 전부 추천 금지 — 하락추세 종목은 반등 기대해도 시간 걸림
...
## 투자 유형별 추천 (총 9종목)
- 단타 (1일 목표): 추세↑ 필수 + RSI 35~60 + MACD↑ 필수 + BB하단 근접 또는 눌림목 반등, 매수가 = 당일 시가 예상
  * 볼린저밴드 하단 근접(buy) 종목 최우선 / 캔들패턴 hammer·doji 우선
  * 오늘 뉴스에 직접 언급된 종목 단타 금지 (이미 선반영) — 뉴스 미언급 기술적 신호 종목 발굴
  * 추세↓ 종목 단타 절대 금지
  * RSI 60 초과 종목 단타 절대 금지 (강세장 불장 시 65까지 예외)
- 스윙 (3~5일 목표): 추세↑ 필수 + RSI 40~75 + MACD↑ 또는 거래량급증, 매수가 = 전일 종가
  * 추세↓ 종목 스윙 금지 / 지지선 근접 종목 우선 / 불장 강세장에서만 RSI 80까지 예외 허용
- 중기 (2~4주 목표): 추세↑ 필수 + ROE 8%↑ + PBR 3 이하 + RSI 70 미만, 매수가 = 전일 종가
  * 추세↓ 종목 중기 금지 / PER 낮고 PBR 1 이하면 가산점
...
- stop_loss: 단타는 매수가×0.97(-3%), 스윙은 매수가×0.95(-5%), 중기는 매수가×0.92(-8%)
{
  "recommendations": [
    {
      "name": "종목명",
      ...
      "buy_price": 매수가격(숫자),
      "sell_price": 목표매도가(숫자),
      "stop_loss": 손절가(숫자 — 단타 -3%, 스윙 -5%, 중기 -8%),
